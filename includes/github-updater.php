...
class WP_GitHub_Updater {
	...
	public function set_defaults() {
		if ( !empty( $this->config['access_token'] ) ) {
			...
			$zip_url = add_query_arg( array( 'access_token' => $this->config['access_token'] ), $this->config['zip_url'] );
	...
	public function get_new_version() {
		...
		if ( $this->overrule_transients() || ( !isset( $version ) || !$version || '' == $version ) ) {
			...
			if ( false !== $version )
				set_site_transient( $this->config['slug'].'_new_version', $version, 60*5 ); // 60*60*6
		}
	...
	public function remote_get( $query ) {
		if ( ! empty( $this->config['access_token'] ) )
			$query = add_query_arg( array( 'login' => 'bradmkjr', 'access_token' => $this->config['access_token'] ), $query );

		$args = array(
			'sslverify' => $this->config['sslverify'],
			'headers' => array( 'access_token' => $this->config['access_token'] )
		);
	 ...
	 * @return mixed
	 */
	public function remote_api_get( $query ) {
		...
		$raw_response['body'] = base64_decode($json_decoded->content);
